...
<nav class="flex items-center justify-between px-6 md:px-10 py-4 bg-white border-b">
    ...
    <div class="hidden md:flex items-center gap-10 text-sm font-medium text-gray-600">
        <a href="{{ route('admin.dashboard') }}" class="hover:text-green-800">Dashboard</a>
        <a href="{{ route('admin.produk') }}" class="text-green-800 border-b-2 border-green-800 pb-1">Produk</a>
        <a href="{{ route('admin.pesanan') }}" class="hover:text-green-800">Pesanan</a>
    </div>
...
<div class="px-10 py-8 flex justify-between items-start">
    <div>
        ...
        <p class="text-sm text-gray-600 mt-2">Kelola seluruh data produk dengan mudah sesuai kebutuhan.</p>
    </div>
...
<div class="px-10 flex gap-8">
    ...
    <div class="w-1/4">
        ...
        <div class="space-y-2 text-sm text-gray-600">
            <label class="flex gap-2 cursor-pointer">
                <input type="checkbox" value="tenda" onchange="filterKategori()"> Tenda &amp; Camping
            </label>
            <label class="flex gap-2 cursor-pointer">
                <input type="checkbox" value="tas" onchange="filterKategori()"> Tas &amp; Carrier
            </label>
            <label class="flex gap-2 cursor-pointer">
                <input type="checkbox" value="pakaian" onchange="filterKategori()"> Pakaian Gunung
            </label>
            <label class="flex gap-2 cursor-pointer">
                <input type="checkbox" value="aksesoris" onchange="filterKategori()"> Aksesoris &amp; Gear
            </label>
        </div>
        ...
        <div class="mt-6 bg-[#eef0ea] p-4 rounded-xl text-sm max-w-[220px]">
            ...
            <h4 class="font-bold text-[#213B56] text-sm mb-2">Peralatan Terjamin</h4>
    ...
    <div class="w-3/4">
        ...
        <!-- SEARCH -->
        <form method="GET" action="{{ route('admin.produk') }}" class="mb-6 flex gap-2">
            <input type="text"
                name="search"
                id="searchInput"
                value="{{ request('search') }}"
                placeholder="Cari barang..."
                class="w-full border p-3 rounded-lg bg-[#EDEFE7] outline-none focus:ring-2 focus:ring-green-700">
            <button type="submit"
                class="bg-green-800 text-white px-5 py-3 rounded-lg text-sm font-semibold hover:bg-green-900 whitespace-nowrap">
                Cari
            </button>
            @if(request('search'))
                <a href="{{ route('admin.produk') }}"
                   class="bg-gray-200 text-gray-700 px-4 py-3 rounded-lg text-sm font-semibold hover:bg-gray-300 whitespace-nowrap flex items-center">
                    Reset
                </a>
            @endif
        </form>
        ...
        <div class="grid grid-cols-3 gap-6" id="produkGrid">
            ...
            <div class="produk-item" data-nama="{{ strtolower($alat->nama_alat) }}">
                <div class="group bg-[#efefe9] rounded-xl shadow overflow-hidden hover:shadow-xl hover:-translate-y-1 transition duration-300">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex justify-between items-end">
                            <div>
                                ...
                                <p class="text-green-800 font-bold text-lg">
                                    Rp {{ number_format($alat->harga_per_hari, 0, ',', '.') }}
                        ...
                        <p class="text-xs text-gray-500 mt-3">Stok: {{ $alat->stok }}</p>
                    </div>
                </div>
            </div>
            @empty
            <div class="col-span-3 text-center py-16 text-gray-400">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.produk') }}" class="text-green-700 font-semibold hover:underline mt-2 inline-block">← Lihat semua produk</a>
                @else
        ...
        <div class="flex justify-center items-center gap-2 mt-10 mb-16">
            ...
            @if($alats->onFirstPage())
                <span class="w-10 h-10 flex items-center justify-center rounded-lg border bg-gray-100 text-gray-400 cursor-not-allowed">
                    &lt;
                </span>
            @else
                <a href="{{ $alats->previousPageUrl() }}&{{ http_build_query(request()->except('page')) }}"
            ...
            {{-- Nomor Halaman --}}
            @for($i = 1; $i <= $alats->lastPage(); $i++)
                @if($i == $alats->currentPage())
                    <span class="w-10 h-10 flex items-center justify-center rounded-lg bg-green-800 text-white font-semibold">
                        {{ $i }}
                    </span>
                @else
                    <a href="{{ $alats->url($i) }}&{{ http_build_query(request()->except('page')) }}"
                       class="w-10 h-10 flex items-center justify-center rounded-lg border bg-white hover:bg-gray-100">
                        {{ $i }}
                    </a>
                @endif
            ...
            @if($alats->hasMorePages())
                <a href="{{ $alats->nextPageUrl() }}&{{ http_build_query(request()->except('page')) }}"
                   class="w-10 h-10 flex items-center justify-center rounded-lg border bg-white hover:bg-gray-100">
                    &gt;
                </a>
            @else
    ...
    <div class="max-w-7xl mx-auto grid md:grid-cols-3 gap-10">
        ...
        <div>
            ...
            <div class="space-y-3 text-sm text-gray-300">
                <div class="flex items-center gap-3"><span>halo@peakrent.com</span></div>
                <div class="flex items-center gap-3"><span>+62 21 5550 1234</span></div>
            </div>
        </div>
    </div>
...
<div id="deleteModal"
    ...
    <div class="bg-white w-[360px] rounded-xl p-8 text-center relative shadow-xl">
        ...
        <button onclick="closeDeleteModal()"
            class="absolute top-4 right-5 p-1 text-gray-500 hover:text-gray-800 transition" aria-label="Tutup">
...
<script>
// ── Filter kategori (client-side, hanya untuk halaman saat ini) ──
function filterKategori() {
    // Kategori tidak ada di DB, filter dinonaktifkan
    // Bisa diaktifkan kembali jika kolom kategori ditambahkan
}
